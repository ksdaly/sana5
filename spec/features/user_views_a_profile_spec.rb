...
# * An unauthenticated user can not create or access any health profiles and is redirected to signup/login
  ...
  scenario 'unauthenticated user is unable to view a health profile' do
    visit health_profile_path(user1.health_profiles.first)
    expect(page).to have_content("You need to sign in or sign up before continuing")
  end

  scenario 'authorized user is able to view a health profile' do
    sign_in_as(user1)
    visit health_profile_path(user1.health_profiles.first)
    expect(page).to have_content("profile for #{user1.username}")
  end

  scenario 'unauthorized user is unable to view a health profile' do
    sign_in_as(user2)
    visit health_profile_path(user1.health_profiles.first)
